...
## 2026-05-03: Category P - AI Context & Specific Instructions
Implemented all ideas from Category P to improve the payload sent to AI agents:
- **Idea 76 (Contextuele Code Snippets):** Worker now fetches the target file from GitHub using `octokit.rest.repos.getContent` when a review comment is bound to a file/line. It injects a +/- 5 lines context snippet directly into the bot comment markdown.
- **Idea 77 (PR Description & Intent):** Modified `formatAggregatedBody` and `worker.ts` to fetch the PR title and description directly via the GitHub API and include it at the top of the aggregated comment context.
- **Idea 78 ("Related Files" Map):** The worker runs a regex extraction (`/(?:import|require)\s*\(?\s*['"]([^'"]+)['"]/g`) on the fetched file content to list related/imported files, appending this list to the context snippet.
- **Idea 79 (Project Architectuur Samenvatting):** Added `architectureInfo` field to the Prisma `Repository` model, exposed this field in the Next.js API, and created a textarea input in the `Repositories` UI page. The worker injects this info into the final payload context.
- **Idea 80 (Vorige Mislukte Fix-Pogingen Log):** The worker queries the Prisma database for previous AI tasks (where `julesSessionState` is 'COMPLETED') and previous forwarded aggregated comments on the same PR. If found, these attempts are included inside a `<details>` collapsible block in the AI payload to provide history and prevent infinite loops.
- Implemented Q category from IDEAS.md including AgentFeedback, FlakyTestRule, ErrorRewriteRule, and ResolutionTime models. Also added Learning UI in src/app/learning
