...
export default function RepositoriesPage() {
  ...
  const onSaveEdit = async (data: Repo) => {
    await fetch(`/api/repositories/${data.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
          ...
          <div>
            ...
            <input type="number" min="0" {...register('requiredApprovals')} className="w-full px-4 py-2 border border-gray-300 rounded-md" />
          </div>
          <div>
                ...
                {editingId === repo.id ? (
                    ...
                    <form onSubmit={handleSubmitEdit(onSaveEdit)} className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div>
                          ...
                          <input type="number" min="0" {...registerEdit('requiredApprovals')} className="w-full px-2 py-1 border rounded" />
                        </div>
                        <div>
                  ...
                  <>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button onClick={() => startEdit(repo)} className="text-blue-600 hover:text-blue-900 mr-4">
                        <Edit2 size={18} />
                      </button>
